...
public class DrawBlueSomething extends SakuraItem {
    public DrawBlueSomething(Settings settings) {
        super(settings);
    }
    ...
    public void itemEntityTick(ItemEntity itemEntity, World world, ItemStack itemStack) {
        if (world instanceof ServerWorld serverWorld) {
            ...
            if (tickCount >= 5) {
                tickCount = 0;
                ParticleEffect particleEffect = new DustParticleEffect(new Vector3f(0, 1, 1), 1);
                serverWorld.spawnParticles(particleEffect, itemEntity.getX(), itemEntity.getY() + 0.5, itemEntity.getZ(), 300, 1.3, 0.5, 1.3, 0.02);
            }
            getItemStack(itemEntity, serverWorld, customNbt1, nbtCompound, tickCount, tickCount2);
        }
    }

    public static void getItemStack(ItemEntity itemEntity, ServerWorld serverWorld, LivingEntityCustomNbt customNbt1, NbtCompound nbtCompound, int tickCount, int tickCount2) {
        if (tickCount2 >= 25) {
            tickCount2 = 0;
